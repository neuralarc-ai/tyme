...
export function InviteDialog({ meetingTime, meetingDate, timezone }: InviteDialogProps) {
  ...
  // Automatically generate Meet link when dialog opens
  useEffect(() => {
    if (isOpen && !formData.meetingLink) {
      (async () => {
        ...
        try {
          const response = await fetch('/api/send-invite', {
            ...
            body: JSON.stringify({
              senderName: formData.senderName || 'Tyme User',
              senderEmail: formData.senderEmail || 'noreply@tyme.neuralarc.ai',
              recipientEmails: formData.recipientEmails || 'noreply@tyme.neuralarc.ai',
              description: formData.description || '',
              meetingTime: meetingTime,
              meetingDate: meetingDate,
              timezone: timezone,
              onlyGenerateMeetLink: true
            })
          })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.meetingLink || !formData.meetingLink.includes("meet.google.com")) {
        throw new Error("Please provide a valid Google Meet link (generate or paste)")
      }
      const response = await fetch('/api/send-invite', {
        method: 'POST',
        headers: {
      ...
      <Dialog open={isOpen} onOpenChange={(open) => { setIsOpen(open); setSuccessMessage("") }}>
        ...
        <DialogContent className="sm:max-w-[425px] bg-black text-white border border-white">
          ...
          <form onSubmit={handleSubmit} className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              <Label htmlFor="meetingLink" className="text-white flex items-center gap-2">
                Google Meet Link
              </Label>
              <Input
                id="meetingLink"
                ...
                placeholder="Paste or generate a Google Meet link"
                required
                className="bg-black text-white border-white border placeholder-gray-400"
                disabled={isGenerating}
              />
              <p className="text-sm text-gray-400">You can paste your own Meet link or use the generated one.</p>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              ...
              <Button type="submit" disabled={isLoading || isGenerating} className="bg-white text-black border border-white hover:bg-black hover:text-white transition-colors">
                {isLoading ? (
                  <>
        ...
        <Dialog open={true} onOpenChange={() => setSuccessMessage("")}> 
          <DialogContent className="sm:max-w-[400px] bg-white text-black border border-black">
            <div className="flex flex-col items-center justify-center p-6 text-center">
              ...
              <Button className="mt-2 bg-black text-white border border-white hover:bg-white hover:text-black transition-colors" onClick={() => setSuccessMessage("")}>Close</Button>
            </div>
          </DialogContent>
        </Dialog>
      )}
